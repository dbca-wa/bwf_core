...
from bwf_core.workflow.dto.component_dto import ComponentDto
...
class Workflow(models.Model):
    ...
    workflow_type = models.CharField(
        max_length=50,
        choices=WorkflowTypesEnum.choices,
        default=WorkflowTypesEnum.SHORT_LIVED,
    )
...
class WorkflowVersion(models.Model):
    workflow = models.ForeignKey(
        to=Workflow, on_delete=models.CASCADE, related_name="versions"
    )
    ...
    workflow_file = models.FileField(
        max_length=1000,
        upload_to=updaload_to_workflow_edition_path,
        null=True,
        blank=True,
        storage=upload_storage,
    )
    ...
    def deactivate_version(self):
        if self.is_active:
            raise Exception(
                "Cannot deactivate an active version. Please set another version as active first."
            )
        self.is_disabled = True
        self.disabled_at = self.updated_at
        self.save()
...
class WorkFlowInstance(models.Model):
    ...
    status = models.CharField(
        max_length=50,
        choices=ComponentStepStatusEnum.choices,
        default=ComponentStepStatusEnum.PENDING,
    )
    ...
    def is_active(self):
        return self.status in [
            ...
            WorkflowStatusEnum.AWAITING_ACTION,
        ]
...
class ComponentInstance(models.Model):
    ...
    workflow = models.ForeignKey(
        WorkFlowInstance, on_delete=models.CASCADE, related_name="child_actions"
    )
    ...
    status = models.CharField(
        max_length=50,
        choices=WorkflowStatusEnum.choices,
    ...
    def get_input_values(self, inputs=[], context_values={}):
        ...
        for input in inputs:
            values[input.key] = {
                "key": input.get("key"),
                "value": input.get_value(context_values),
...
class WorkflowInstanceFactory:
    ...
    def create_instance(workflow_version: WorkflowVersion, input_params={}):
        ...
        for key in input_values:
            ...
            input_value = WorkflowInstanceFactory.__get_input_value(
                input, input_params.get(input["key"])
            )
    ...
    def __get_input_value(input_value, param_value=None):
        ...
        json_default = input_value.get(
            "default_value", None
        )  # TODO: get default type for data type
        json_value = input_value.get("value", {})

        if (
            param_value is None
            and input_value["required"] == True
            and (json_default is None or json_default.get("value") is None)
        ):
        ...
        if param_value is None:
            ...
            if input_type in [
                WorkflowInputTypesEnum.SELECT,
                WorkflowInputTypesEnum.MULTI_SELECT,
            ]:
                values = []
                param_value = (
                    param_value
...
class ActionLog(models.Model):
    ...
    component_instance = models.ForeignKey(
        to=ComponentInstance,
        on_delete=models.SET_NULL,
        related_name="action_logs",
        null=True,
    ...
    status = models.CharField(
        max_length=100,
        choices=ActionLogStatusEnum.choices,
        default=ActionLogStatusEnum.STARTED,
    )
